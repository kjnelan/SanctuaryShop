...
class OrderController extends FormController
{
    ...
    public function reissueDownloads(): void
    {
        Session::checkToken() or jexit('Invalid Token');
        $orderId = $this->input->getInt('id');
        try {
            $this->getModel('Order')->reissueDownloads($orderId);
            $this->setMessage('Fresh download links were generated and emailed to the customer.', 'success');
        } catch (\Throwable $e) {
